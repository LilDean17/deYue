...
- **基于响应相似度区分越权类型**：对同一请求发送高权限（A）、低权限（B）、未授权（W）三组请求，通过 Levenshtein 相似度算法对比响应内容：
  - **A vs B 相似度 ≥ 98%** → 判定为 **IDOR**（水平越权）
  - **A vs 未授权 相似度 ≥ 98%** → 判定为 **Unauthorized**（未授权访问）
  - 可同时命中两者
  - 检测阈值 98%，表格中相似度 ≥ 95% 时显示 ✔ 标记
...
- **EDT 线程安全**：检测请求在后台线程执行，Swing 组件更新全部通过 `SwingUtilities.invokeLater` 在 EDT 执行；未授权认证字段通过 `volatile` 变量缓存，供后台线程安全读取。
...
注：相似度 ≥ 95% 时表格中会显示 ✔ 标记（仅视觉提示，不影响判定）。
